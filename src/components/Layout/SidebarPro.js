...
import { PrivacyPolicy } from './SidebarIcons/PrivacyPolicy';
 ...
 const SidebarPro = () => {
    ...
    const active = {
        dashboard: location?.pathname == "/admin/dashboard",

        userManagement: location?.pathname == "/admin/user-list",
        userList: location?.pathname == "/admin/user-list",
 
        eventManagement: location?.pathname == "/admin/event-list" ,
        allEvents: location?.pathname == "/admin/event-list"  ,
 

        contentManagement: location?.pathname == "/admin/terms-and-conditions" || location.pathname == "/admin/privacy-policy" || location.pathname == "/admin/about-us",
        termsAndConditions: location?.pathname == "/admin/terms-and-conditions",
        privacyPolicy: location?.pathname == "/admin/privacy-policy", 
    }
    return (
            ...
            <Sidebar image="https://i.pinimg.com/736x/8e/6c/06/8e6c064f57f94838263d7ba9ad80f353.jpg" rtl={false}
                ...
                <div className="sidebar-header">
                    <div className="logo text-center">
                        <Link to="/admin/dashboard"><img src="/assets/images/logo1.png" style={{ width: "50%", borderRadius: 10 }} alt="logo" className="img-fluid my-3" /></Link>
                    </div>
                </div>
                <Menu menuItemStyles={{
                    root: {
                    ...
                    <Link to="/admin/dashboard" style={{ paddingLeft: 0, color: '#8ba1b7' }}>
                    ...
                    <SubMenu SubMenuExpandIcon defaultOpen={active?.userManagement} active={active?.userManagement} icon={<User />} label="Users Management" >
                        <MenuItem component={<Link to="/admin/user-list" />} active={active?.userList} icon={<AllUsers />} rootStyles={{
                    ...
                    <SubMenu defaultOpen={active?.eventManagement} active={active?.eventManagement} icon={<Specie />} label="Events Management" >
                        <MenuItem component={<Link to="/admin/event-list" />} active={active?.allEvents} icon={<Specie />} rootStyles={{
                    ...
                    <SubMenu defaultOpen={active?.contentManagement} active={active?.contentManagement} icon={<Content />} label="Content Management" >
                        <MenuItem component={<Link to="/admin/terms-and-conditions" />} active={active?.termsAndConditions} icon={<TermsAndConditions />} rootStyles={{
                            backgroundColor: active?.termsAndConditions ? "rgba(0, 69, 139,1)" : '#1D454C',
                            fontSize: '15px',
                        }}> Terms & Conditions </MenuItem>
                        <MenuItem component={<Link to="/admin/privacy-policy" />} active={active?.privacyPolicy} icon={<PrivacyPolicy />} rootStyles={{
